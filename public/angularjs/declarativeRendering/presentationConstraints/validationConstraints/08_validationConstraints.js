...
angularjsApp.factory('validationConstraintsData', function () {
  const data = {
    section: {
      ...
      spec: "1.2.2. Validierungsbedingungen"
    },
    article: {
      ...
      codeSnippets: [{
        ...
        lang: "js",
        code:
`app.controller('ValidationConstraintsCtrl', ['$scope', function ($scope) {
  $scope.fealdValidation = {
    required: true,
    minLength: 8,
    pattern: /^(?=.*[A-Za-z])(?=.*\d)(?=.*[$@$!%*#?&])[A-Za-z\d$@$!%*#?&]{8,}$/,
...
<form name="form">
  ...
  <div class="form-group">
    ...
    <input type="password" ng-model="password" class="form-control" id="inputPass" name="inputPass"
      ng-required="fealdValidation.required" ng-maxlength="fealdValidation.maxLength"
      ng-minlength="fealdValidation.minLength" ng-pattern="fealdValidation.pattern" />
  ...
  <p>
    <pre><code>{{form.inputPass}}</code></pre>
  </p>
</form>
</div>`
      },
